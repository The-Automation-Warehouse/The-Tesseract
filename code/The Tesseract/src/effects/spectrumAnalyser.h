...
void spectrumAnalyser(CRGB pixels[NUM_LEDS], int inputPin)
{
  ...
  for (byte band = 0; band < NUM_BANDS; band++)
  {
    ...
    Serial.println(oldBarHeight[band]);
    Serial.println(" ");
    ...
    if (band == 1 || 2 || 3) {
      if (barHeight > 5)
      {
        barHeight = ceil(oldBarHeight[band] * 0.7 + barHeight * 0.3);
      } else {
        barHeight = oldBarHeight[band] * 0.7 + barHeight * 0.3;
      }
    } else if (band == 4 || 5) {
      if (barHeight > 5)
      {
        barHeight = ceil(oldBarHeight[band] * 0.2 + barHeight * 0.8);
      } else {
        barHeight = oldBarHeight[band] * 0.8 + barHeight * 0.2;
      }
    }
